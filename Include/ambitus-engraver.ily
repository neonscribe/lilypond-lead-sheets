...
#(define-method (update-ambitus-notes (ambitus <ambitus>) note-grob)
   "Update the upper and lower ambitus pitches of @var{ambitus}, using
@var{note-grob}."
...
- both note heads are added to the ambitus line grob, so that a line should
  be printed between them."
   ...
   (if (and (ambitus-note-pitch (ambitus-note ambitus UP))
    ...
    (for-each (lambda (direction msg)
	       (let* ((note (ambitus-note ambitus direction))
		      (pitch (ambitus-note-pitch note)))
		(ly:message msg 
		 pitch)))
     (list DOWN UP)
     '("low note ~a" "high note ~a"))
	...
	(ly:message "No notes")
         (for-each (lambda (direction)
                   ...
                   (list DOWN UP))
         (ly:grob-suicide! (ambitus-line ambitus)))))
